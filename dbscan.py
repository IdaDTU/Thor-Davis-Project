...
def filter_and_merge_clusters2(df, min_clusters=1, max_duration=0.5, time_tolerance=0.01, frame_rate=40):
    """
    Filter clusters based on the number of events and duration, and merge clusters with similar mean times.
    ...
    # Merge close-in-time and close-in-space clusters
    merged_clusters = []
    merged_mean_times = []
    merged_mean_positions = []  # Store (x, y) mean positions

    for i, mean_time in enumerate(short_mean_times):
        indices = np.where(labels_valid == short_clusters[i])[0]
        found = False
    
    # Calculate mean position for this cluster
        cluster_x = np.array(x_valid)[indices]
        cluster_y = np.array(y_valid)[indices]
        mean_x = np.mean(cluster_x)
        mean_y = np.mean(cluster_y)

    for j, existing_time in enumerate(merged_mean_times):
        time_diff = abs(mean_time - existing_time)
        # Calculate distance to the existing merged cluster's mean position
        existing_x, existing_y = merged_mean_positions[j]
        spatial_distance = np.sqrt((mean_x - existing_x)**2 + (mean_y - existing_y)**2)

        if time_diff <= time_tolerance and spatial_distance <= 20:
            merged_clusters[j].extend(indices)
            # Update mean position (optional, could refine here)
            found = True
            break
    ...
    if not found:
        merged_clusters.append(list(indices))
        merged_mean_times.append(mean_time)
        merged_mean_positions.append((mean_x, mean_y))
